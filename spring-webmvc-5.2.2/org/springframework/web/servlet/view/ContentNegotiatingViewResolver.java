 ...
 */
// 视图解析器：根据方法的返回值得到试图对象（View），视图对象决定如何渲染（转发？重定向？）
// 组合所有的视图解析器
public class ContentNegotiatingViewResolver extends WebApplicationObjectSupport
	...
	@Override
	protected void initServletContext(ServletContext servletContext) {// 初始化
		Collection<ViewResolver> matchingBeans =
				BeanFactoryUtils.beansOfTypeIncludingAncestors(obtainApplicationContext(), ViewResolver.class).values();// 使用BeanFactoryUtils从容器中获取所有的视图解析器，因此可以将自己定义的视图解析器添加到容器中，contentnegotiatinviewresolver就会自动将其组合进来
	...
	@Override
	@Nullable
	public View resolveViewName(String viewName, Locale locale) throws Exception {// 解析试图方法
		RequestAttributes attrs = RequestContextHolder.getRequestAttributes();
		Assert.state(attrs instanceof ServletRequestAttributes, "No current ServletRequestAttributes");
		List<MediaType> requestedMediaTypes = getMediaTypes(((ServletRequestAttributes) attrs).getRequest());
		if (requestedMediaTypes != null) {
			List<View> candidateViews = getCandidateViews(viewName, locale, requestedMediaTypes);// 获取候选视图对象
			View bestView = getBestView(candidateViews, requestedMediaTypes, attrs);// 选择最适合的试图对象，并返回
			if (bestView != null) {
				return bestView;
			}
		}
	...
	private List<View> getCandidateViews(String viewName, Locale locale, List<MediaType> requestedMediaTypes)
		...
		if (this.viewResolvers != null) {
			Assert.state(this.contentNegotiationManager != null, "No ContentNegotiationManager set");
			for (ViewResolver viewResolver : this.viewResolvers) {// 拿到所有视图解析器挨个解析
